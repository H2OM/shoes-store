<?php
    ...
    use app\Db;
    class CatalogingSFS extends Runner {
        protected function getCatalog() {
            if(isset($_GET['fav']) && (!isset($_SESSION['user']) || empty($_SESSION['user']))) {header("HTTP/1.0 400 Bad request");die;}
            $catalog = HgetFilteredCatalog();
            if(isset($catalog['id'])) $catalog = [$catalog];
            $filters = HGetFilters();
            exit(json_encode(["goods"=>$catalog, "count"=>count($catalog),"filters"=>$filters]));
        }
        protected function getProduct() {
            if(!isset($_GET['category']) || !isset($_GET['article'])) {header("HTTP/1.0 400 Bad request");die;}
            $prepaired = [["VALUE"=>HgetSafeString($_GET['article']), "PARAMVALUE"=>13],["VALUE"=>HgetSafeString($_GET['article']), "PARAMVALUE"=>13],["VALUE"=>HgetSafeString($_GET['article']), "PARAMVALUE"=>13],["VALUE"=>HgetSafeString($_GET['article']), "PARAMVALUE"=>13]];
            try {
                ...
                if( !empty($product['Colors'])) {
                    ...
                    foreach(explode(',', $product['Colors']) as $k=>$v) {
                        if(!in_array($v, $colors)) array_push($colors, $v);
                    }
                    sort($colors);
