<?php
    ...
    use app\Db;
    class BasketingARC extends Runner {
        ...
        protected function addBasket() {
            if(!isset($_POST['article']) || !isset($_POST['size'])) {header("HTTP/1.0 403 Bad request");die;}
            try {
        ...
        protected function setCount() {
            if(!isset($_POST['article']) || !isset($_POST['type']) || !isset($_POST['size'])) {header("HTTP/1.0 403 Bad request");die;}
            foreach($_SESSION['basket'] as $k=>$v) {
                if($v['Article'] == $_POST['article'] && $v['Size'] == $_POST['size']) {
                    switch($_POST['type']) {
                        case "less":
                            if((string)$_SESSION['basket'][$k]['Value'] === (string)1) array_splice($_SESSION['basket'], $k, 1);
                                else $_SESSION['basket'][$k]['Value']--;
                            break;
                        case "more":
                            $_SESSION['basket'][$k]['Value']++;
                            default: 
        ...
        protected function removeProd() {
            if(!isset($_POST['article']) || !isset($_POST['size'])) {header("HTTP/1.0 403 Bad request");die;}
            foreach($_SESSION['basket'] as $k=>$v) {
                if($_SESSION['basket'][$k]['Article'] == $_POST['article'] && $_SESSION['basket'][$k]['Size'] == $_POST['size']) {
