<?php
    ...
    use app\Db;
    class UseringARDP extends Runner {
        private $isAutorizated;
        ...
        protected function isAuth() {
            exit(json_encode($this->isAutorizated));
        }
        protected function signIn() {
            if($this->isAutorizated) {header("HTTP/1.0 405 Method Not Allowed"); die;}
            if(count($_POST) != 2) {header("HTTP/1.0 400 Bad request");die;}
            $userData = HcheckUserInput();
            if(empty($userData) || (!isset($userData['number'], $userData['password']) && count($userData) != 2)) {header("HTTP/1.0 400 Bad request");die;}
            $attributes = HprepairedQuery(["number"=>$userData['number']]);
            try {
                $result = Db::getPreparedQuery("SELECT * FROM `users` WHERE `Phone` = ?", $attributes);
                if(empty($result)) {exit(json_encode("User not found"));}
                ...
                } else {exit(json_encode("Uncorected password"));}
        ...
        protected function logOut() {
            if(!$this->isAutorizated) {header("HTTP/1.0 401 Unauthorized"); die;}
            HdestroySession();
            exit(json_encode(true));
        ...
        protected function signUp() {
            if($this->isAutorizated) {header("HTTP/1.0 405 Method Not Allowed"); die;}
            if(count($_POST) != 9) {header("HTTP/1.0 400 Bad request");die;}
            $userData = HcheckUserInput();
            if(empty($userData) || (!isset($userData['firstName'], $userData['secondName'], $userData['number'],  
                                          $userData['password'], $userData['age'], $userData['gender'], $userData['mail']) && count($userData) != 7)) {header("HTTP/1.0 400 Bad request");die;}
        ...
        protected function editUser() {
            // sleep(5);
            if(!$this->isAutorizated) {header("HTTP/1.0 401 Unauthorized"); die;}
            if(count($_POST) != 6) {header("HTTP/1.0 400 Bad request");die;}
            $userData = HcheckUserInput();
            if(empty($userData) || (!isset($userData['firstName'], $userData['secondName'], $userData['number'],  
                                    $userData['age'], $userData['gender'], $userData['mail']) && count($userData) != 6)) {header("HTTP/1.0 403 Bad request");die;}
            $attributes = HprepairedQuery($userData);
            try {
        ...
        protected function changeFav() {
            if(!$this->isAutorizated) {header("HTTP/1.0 401 Unauthorized"); die;}
            if(!isset($_GET['article']) || !isset($_GET['action'])) {header("HTTP/1.0 403 Bad request");die;}
            $sql = "";
            switch($_GET['action']) {
                case "set":
            ...
            try {
                ...
                if(isset($_SESSION['basket']) && !empty($_SESSION['basket'])) {
                    foreach($_SESSION['basket'] as $k=>$v) {
                        if(($v['Article'] == $_GET['article']) && isset($_SESSION['basket'][$k]['Fav'])) $_SESSION['basket'][$k]['Fav'] = !$v['Fav'];
                    }
