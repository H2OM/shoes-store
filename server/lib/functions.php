<?php
        ...
        foreach($data as $k=>$v) {
            ...
            switch($k) {
                ...
                case "number":
                    ...
                    break;
                default:
                    if(is_numeric($v)) array_push($preparedQuery, ["VALUE"=>HgetSafeString($v), "INT"=>true]);
                        else array_push($preparedQuery, ["VALUE"=>HgetSafeString($v), "PARAMVALUE"=>128]);
                    break;
            }
        }
    ...
    function HcheckUserInput() {
        ...
        foreach($_POST as $key => $value) {
            ...
            switch($key) {
                case "number": 
                    if(!preg_match("/^\+7\s\(\d{3}\)\s\d{3}-\d{2}-\d{2}$/", $value)) return false;
                ...
                case "password":
                    ...
                    if(isset($_POST["rePassword"]) && $_POST["rePassword"] !== $value) return false;
                    $dataInputs[$key] = $value;
                    break;
                case "firstName":
                ...
                case "secondName":
                    ...
                    break;
                case "age":
                    if(!is_numeric($value) || ($value < 6 && $value > 150)) return false;
                    $dataInputs[$key] = $value;
                    break;
                case "gender":
                    switch($value) {
                    ...
                    break;
                case "mail":
                    if(!filter_var($value, FILTER_VALIDATE_EMAIL)  || preg_match("/'/", $value)) return false;
                    $dataInputs[$key] = $value;
                    break;
                    case "rePassword":
                        if(iconv_strlen($value) < 6 || !preg_match("/\D+/", $value) || strtolower($value) == $value || strtoupper($value) == $value) {
                        ...
                        if(isset($_POST["password"]) && $_POST['password'] != $value) return false;
                        break;
                case "agreement":
                    if($value !== "on") return false;
                    break;
                case "title":
                    if(iconv_strlen($value) > 30 || iconv_strlen($value) < 5) return false;
                    $dataInputs[$key] = $value;
                    break;
                case "message":
                    if(iconv_strlen($value) > 300 || iconv_strlen($value) < 5) return false;
                    $dataInputs[$key] = $value;
                    break;
                    default: break;
            }
    ...
    function HgetFilteredCatalog() {
        ...
        if(isset($_GET) && !empty($_GET)) {
            ...
            foreach($_GET as $k=>$v) {
                ...
                switch($k) {
                    ...
                    case "sale":
                        ...
                        break;
                    case "fav":
                        if(!isset($_SESSION['user']) || empty($_SESSION['user'])) break;
                        switch($v) {
                            case "true":
            ...
            if(strlen($sql_part_condition) > 0) $sql_part_condition = " WHERE ". substr_replace($sql_part_condition, "", -5);
    ...
    function HGetFilters() {
        ...
        try {
            $result = Db::getQuery("SELECT filters.Filter, filters.Code, filters.Type, filters_values.id,
                                    ...
                                    AND categories.id = goods.Category_id ".$sql_part_condition." ".((isset($_GET['fav']) && $_GET['fav'] == 'true') ? "JOIN favorites ON favorites.Goods_id = goods.id" : "")."
            ...
            foreach($result as $key=>$value) {
                if($value['Name'] == "" && $value['ValueCode'] == "") continue;
                if(!isset($filters[$value['Type']])) $filters[$value['Type']] = [];
                $index = "";
                ...
                if($index !== "") {
                    ...
                    array_push($filters[$value['Type']], ["Filter"=>$value['Filter'], "Code"=>$value['Code'], "Values"=>[["Name"=>$value["Name"], "ValueCode"=>$value['ValueCode']]]]);
                }
            }
            return $filters;
        } catch(\PDOException $e) {
    ...
    function reloadBasket() {
        if(!isset($_SESSION['basket']) || empty($_SESSION['basket']) || !isset($_SESSION['user']) || empty($_SESSION['user'])) return;
        $sql_part = "";
            ...
            if(!empty($result)) {
                ...
                foreach($_SESSION['basket'] as $k=>$v) {
                    foreach($result as $key=>$val) {
                        if($v['id'] == $val['id']) $_SESSION['basket'][$k]['Fav'] = $val['Fav'];
                    }
                }
            }
            return;
