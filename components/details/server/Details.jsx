...
export default async function Details ({params}) {
    const {product, related} = await fetch('http://127.0.0.1/api/catalog/get-product?category='+params.type+'&article='+params.article, {cache: "no-cache"})
        .then(data=>{
            if(!data.ok) {
                notFound();
            }
    ...
    let {Title, Type, Brand, Size, Image : img, SliderImages, Price, Article, Color, Price_old: OldPrice, Category, Fav = null, Colors, Description} = product;
            ...
            <div className="grid">
                ...
                <div className="Details__split">
                    <div className="Details__split__content">
                        <div>
                            ...
                            <div className="Details__split__content__subtitle Details__split__content__subtitle_art" >Артикул - {Article}</div>
                            <div className="Details__split__content__subtitle Details__split__content__subtitle_color">Цвет - {Color}</div>
                        </div>
                        { Colors.length > 1 ? 
                            <>
                ...
                <div className="Details__tabs">
                    ...
                    <div className="Details__tabs__content">
                        <div className="Details__tabs__content__block Details__tabs__content__block_active" data-link={"Описание"}>
                            {Description}
                        </div>
                ...
                { Array.isArray(related) && related.length > 0 ? <MiniSlider title={"Похожие товары"} products={related}/> : null }
            </div>
        </section>
    )
}